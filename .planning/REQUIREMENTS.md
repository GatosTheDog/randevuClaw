...
- [ ] **BOT-04**: Telegram client layer is migrated from `node-telegram-bot-api` to Telegraf 4.15+; all existing booking and owner flows continue working unchanged
- [ ] **BOT-05**: PostgreSQL RLS policies enforce per-business DB isolation; cross-tenant row access is impossible even if application code omits a `WHERE business_id` clause
