...
**Budget:** $0 target (free tiers only for PoC)  
**Overall Confidence:** HIGH (all recommendations backed by official docs and 2025-2026 current state)
...
**Installation:**
```bash
npm install @meta-technologies/whatsapp-cloud-api express dotenv
```
...
**SDK Migration Note:** If you find old @google/generative-ai code online, ignore it. The migration from the old SDK to @google/genai is straightforward; Google's docs are updated.
...
- **Multi-Tenant Schema:** Drizzle's `crudPolicy()` helper simplifies RLS policies for Neon:
  ```typescript
  // Each business has a row-level security context; queries auto-filter by business_id
  import { crudPolicy } from 'drizzle-orm/pg-core';
  ```
  This is critical for isolating bookings/conversations between businesses on a single shared platform number.
...
- **Schema Outline (Drizzle):**
  ```typescript
  // businesses: business_id, phone_number, owner_calendar_id, hours_open, hours_close, etc.
  // services: service_id, business_id, name, duration_minutes, price_gr_euros
  // bookings: booking_id, business_id, client_phone, service_id, appointment_time, status
  // conversation_state: conversation_id, business_id, client_phone, context (JSON), expires_at
  ```
...
- **Cron Jobs:** Use Supercronic (already mentioned). Define a `crontab` file:
  ```
  # Send daily agenda at 8:00 AM
  0 8 * * * /app/scripts/send-daily-agenda.sh
  
  # Reminder 30 minutes before appointment (run every 5 min, check DB)
  */5 * * * * /app/scripts/check-and-send-reminders.sh
  ```
  Supercronic is deployed as a separate process in the same Machine; run at scale: 1.
...
# fly.toml
[app]
primary_region = "fra"  # Frankfurt, closest to Greece
...
[processes.cron]
auto_start = true
auto_stop = false  # Always running

[env]
NEON_CONNECTION_STRING = "your-neon-url"
...
All recommendations target free tiers. Here's where the PoC hits the ceiling:
...
**Practical Impact:** The PoC will NOT hit any ceiling if you test with 1–2 businesses. Scaling to 50+ businesses requires moving off free tiers (Gemini → paid, WhatsApp → verification + higher tier), but that's post-PoC.
...
npm init -y
npm install express @meta-technologies/whatsapp-cloud-api @google/genai googleapis \
  drizzle-orm @neondatabase/serverless dotenv zod pino
