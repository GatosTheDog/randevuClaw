...
**Shipped through v1.6 (Telegram Bot UX/Ops Improvements, 2026-07-28); v1.7 Phases 26 (Confirmation & Approval Policy), 27 (Client Consent & Registration), 28 (Admin Menu Discoverability), 29 (Booking & List Clarity), and 30 (Client Identification & Menu Reliability) now also complete — this was the last phase in the v1.7 milestone.** Phase 30 lets owners address clients by name instead of a raw Telegram ID in 4 free-chat tools (view membership, assign to session, send renewal reminder, and list slotless requests — the last found during discussion, same shape as the other 3): a shared `resolveClientByName` helper reuses the existing case-insensitive substring-match convention, disambiguates by name only (never leaking the underlying ID) when 2+ clients match, and is genuinely scoped so a name can never resolve to another business's client. Real Telegram Bot API platform research (not guesswork) confirmed `setChatMenuButton` persists server-side indefinitely but the client-side app cache is the actual reliability gap — the phase closes the code-addressable half regardless (retry+backoff on the one-shot onboarding call, idempotent re-assertion on every `/menu` tap) and documents the remainder as an accepted client-side limitation. A code-review pass caught a real design gap before it could bite in production: two clients sharing an identical Telegram first name would have hit a permanent dead end with no escape hatch (since raw-ID lookup was intentionally removed) — fixed to detect the collision and point the owner at the existing client-list menu instead. Phase 29 closes 5 booking/list UX gaps: same-day session slots whose start time already passed no longer show as bookable in either the `/menu` flow or free-chat tools (owner-facing tools are deliberately unaffected, so owners can still find/cancel/assign a same-day already-started class); admin and client cancel-confirmation prompts show the real date + service/class name instead of a raw internal ID; booking/cancellation/class lists show service names alongside date/time; the client's "Κράτηση μαθήματος" button is relabeled (not silently redirecting) for open-slot businesses; and stale/unknown Telegram callback taps on either menu now show a back-to-menu recovery option instead of a dead end. A code-review pass caught and fixed a real cross-client information-disclosure bug in the same pass — `showCancelConfirm` had zero ownership check, so any Telegram user could have enumerated other clients' booking details via crafted `callback_data`; it now returns a byte-identical generic response for "not found" and "not yours." Phase 28 wires the owner's highest-frequency/highest-stakes actions into `/menu`: a root-level payment-recording button, three Settings example-phrase buttons that surface what free-form Greek phrasing the existing Gemini NLU agent already accepts (hours/services/class setup), an upgraded multi-example "Νέο μάθημα (chat)" button, and a real escalation-reply relay (owner taps "reply", their next message forwards to the escalating client). A code-review pass on Phase 28 caught and fixed a cross-business state leak (the pending-reply relay was keyed only by owner Telegram ID, not business) before it shipped. A brand-new client row now starts unconsented by DB default (and explicitly on insert, defense-in-depth); `/start` and free-form chat both hard-gate on a Ναι/Όχι consent prompt before showing the menu or calling the AI — accepting sets the same `consentGiven` flag that also serves as the opt-in/registration flag, closing the GDPR consent-timing gap (COMP-01) and giving clients a genuine opt-in path (COMP-02). Session-class bookings now require real owner approval (Έγκριση/Απόρριψη keyboard) instead of auto-confirming, with atomic capacity-hold + credit-restore on reject/expiry. Admin can delete a scheduled lesson from the menu or chat; any active bookings on it cascade-cancel with credit restore and a Greek notification to each affected client. Both admin and client get a persistent Telegram menu button (one-tap `/menu`/`/start` access), and when the bot hits its generic Greek error fallback, the owner's own chat now gets a best-effort technical diagnostic (requestId, error type) alongside it. Owners can generate a shareable invite for their business's bot — one message with a QR code (business name + Greek CTA baked into the image) plus the raw `t.me/<bot_username>` deep link as copyable text — from either the admin menu or free chat. The dormant WhatsApp Cloud API integration (webhook, client, config) was fully removed mid-milestone; the bot is Telegram-only with WhatsApp still shelved pending Meta Business Verification. As of Phase 26: client session reschedules now require the same owner approval as new bookings (credit conserved 1-for-1 across a reschedule, fixed post-review), and all 5 destructive owner actions require a real button-tap confirmation whether triggered from the admin menu or free chat.
...
- ✓ Client sees a hard Ναι/Όχι consent gate on first contact via both `/start` and free-form chat — no menu, no AI reply, no persisted conversation turn until accepted — v1.7 Phase 27 (COMP-01)
- ✓ `consentGiven` is a genuine explicit opt-in flag (DB default flipped to `false`, set explicitly on insert too) distinguishing consenting registered clients from incidental first-contact rows — v1.7 Phase 27 (COMP-02)
- ✓ Owner's highest-frequency/highest-stakes actions (record payment, edit hours/services/prices/class setup, reply to escalating client) are all reachable from `/menu`; the "reply to client" button now relays for real, and no dead decorative buttons remain — v1.7 Phase 28 (ADMIN-01, ADMIN-02, ADMIN-03, ADMIN-04)
- ✓ Same-day past-time session slots no longer show as bookable to clients; cancel-confirm prompts and booking/cancellation lists show real date + service/class names instead of raw IDs; the open-slot booking button is relabeled instead of silently redirecting; stale/unknown callback taps show back-to-menu recovery instead of a dead end — v1.7 Phase 29 (UX-01, UX-02, UX-04, UX-05, UX-06)
- ✓ Owner addresses clients by name (not a raw Telegram ID) in 4 free-chat tools, with names-only disambiguation on multi-match and genuine cross-business scoping; Telegram menu-button setup gets retry-with-backoff and a per-`/menu`-tap re-assertion, closing the known code-addressable reliability gaps (real client-side caching limitations documented, not code-fixable) — v1.7 Phase 30 (UX-03, ADMIN-05)

### Active

- [ ] GDPR data-deletion flow (COMP-02/03/04 from earlier milestones, now tracked as COMP-03/COMP-04 — see REQUIREMENTS.md) — deferred v1.1→v1.3, never scheduled into a phase, still not built
