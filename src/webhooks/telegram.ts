  ...
  try {
    ...
    await botTokenStore.run(business.botToken, async () => {
      ...
      const dedupResult = await withBusinessContext(business.id, () =>
        insertOrIgnoreTelegramUpdate(boundUpdateId, business.id, senderTelegramId, updateType)
      );
      logger.info(
      ...
      if (update.callback_query) {
        ...
        if (
          ...
          try {
            ...
            await withBusinessContext(business.id, () => markTelegramUpdateProcessed(boundUpdateId, business.id));
            logger.info(
